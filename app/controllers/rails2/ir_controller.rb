...
class IrController < ApplicationController
  layout "common"
  caches_action :announse,:business,:disclaimer,:highlight,:index,:infomation,:library,:past_library,:market,:message,:stock
  ssl_required :contact,:contact_send
  def index
    render :template => "ir/message"
  end
  ...
  def contact_send
    @ir_message = IrMessage.new(params[:ir_message])
    @ir_message.received_time = Time.now
  ...
  def transrate_irlib
    ...
    # @doc = Hpricot(
    #   Kconv.kconv(
    #     open("http://v3.eir-parts.net/EIR/Eir.aspx?code=#{(params['code'])}&template=#{(params['template'])}&day=#{(params['day'])}&ln=#{(params['ln'])}&year=#{(params['year'])}").read,
    #     Kconv::UTF8,
    #     Kconv::SJIS
    #   )
    # )
    ...
    #BODY要素を取得
    @body = (((@doc/:body)[0]/:table)[0]/:tbody)[0]
    ...
    (@body/:img).each do |img|
      ...
      img["src"] = url_for("site/get_reader.gif") if img["alt"] == "Get Adobe Reader"
    ...
    @body  = Kconv.kconv(@body.to_s,Kconv::UTF8,Kconv::SJIS)
  end
